...
**작성일:** 2026-03-20
**최종 업데이트:** 2026-03-21 (Tier 2 실측 F1=0.8063 + Tier 3 C1/C2 구현 + exception_requested 제거 + 진단 모듈 수정)
...
| **Tier 2 R1+R2 + C5** | B2 중복가중치 + B7 server_env + B1 범주형 + B8 RULE세부 + B9 file_agg + B3 Shape TF-IDF + B6 정규화 + C5 threshold저장 | **0.8063** | **0.75/0.88** | **0.87/0.74** | 538 | **Phase 1 목표 달성** |
| **Tier 3 C1+C2 (구현)** | exception_requested 제거 + C1 Easy FP Suppressor + C2 Slice-aware threshold | - | - | - | ~530 | 서버 실험 대기 |
...
### 1.3 개선 효과 분해 (10M 실데이터 Feature Importance 기반)

| 요인 | 기여도 (실측) | 근거 (10M행 서버 실험) |
|------|-------------|------|
| **B1 범주형 피처** | **매우 높음** | **Importance top 3 독점:** service_enc(2458, 1위), ops_dept_enc(2431, 2위), retention_period_enc(2050, 3위), organization_enc(914, 5위) |
| **B2 중복 샘플 가중치** | 높음 | sample_weight min=0.0070 max=1.1929 → 대량 반복 파일 가중치 대폭 축소 |
| **B3 Shape TF-IDF** | 높음 | tfidf_fshape_* 피처가 중간층에 대거 포진 (9~30위, 134~329) |
| **B6 정규화 강화** | 중간 | min_child_samples=200 + reg_alpha=0.5 + max_depth=10 |
| **B7 server_env 의미 토큰** | 중간 | server_stack_enc(169, 26위). server_is_prod는 top30 밖 (Mock에서 1위였으나 실데이터에서 하향) |
| **B8 RULE 세부, B9 file_agg** | 낮음 | 보조 신호 |

> **주의:** `exception_requested`는 Sumologic에 없어 추론 불가 → Tier 3에서 제거됨. `service`/`ops_dept`/`organization`/`retention_period`는 Sumologic에서 사용 가능 확인됨.
...
### 1.4 현재 피처 구성 (538개, Tier 2 실측)

| 블록 | 피처 수 | 설명 |
|------|--------|------|
| TF-IDF file_name raw | ~200 | char_wb n-gram (max_features=200) |
| TF-IDF file_name shape | ~100 | [B3] _to_shape_text() 변환 후 char_wb n-gram |
| TF-IDF file_path | ~200 | path_text word n-gram (max_features=200) |
| Dense (메타/경로) | ~20 | fname_has_*, pattern_count 통계, 경로 토큰, rule_matched |
| Dense (서버 의미) | 3 | [B7] server_env, server_is_prod, server_stack |
| Dense (RULE 세부) | 3 | [B8] rule_confidence_lb, rule_id_enc, rule_primary_class_enc |
| Dense (file 집계) | 2 | [B9] file_event_count, file_pii_diversity |
| 범주형 _enc | 6 | [B1] service/ops_dept/organization/retention_period/server_env/stack |
| file_extension | 1 | Label Encoded |
| path_features | ~3 | path_depth 등 |

**제거된 피처:**
- `exception_requested` — Sumologic에 없음, 추론 불가 (Tier 3에서 제거)
- `created_hour/weekday/weekend/month` — 시간 과적합 (Tier 1에서 제거)
- `server_freq` — train 통계 누수 (Tier 1에서 제거)
...
| A3 | exception_requested 추가 | ✅→❌ | Tier 1에서 추가 → Tier 3에서 제거 (Sumologic에 없어 추론 불가) |
...
| # | 조치 | 상태 | 근거 |
|---|------|------|------|
| **C1** | **Easy FP Suppressor** — is_system_device/is_package_path+is_mass_detection/is_docker_overlay/has_license_path 조건으로 고확신 FP 선제 분리. Purity 95% 이상 시 활성화. 합산 F1 출력 | ✅ 구현 | 고확신 FP 선제 분리 → hard case에 ML 집중 |
| **C2** | **Slice-aware threshold** — server_env별 Coverage-Precision Curve tau를 개별 계산, threshold_policy.json에 slice_thresholds 저장 | ✅ 구현 | easy slice 커버리지 확대 + hard slice precision 보호 |
| **C5** | **threshold_policy.json 아티팩트 저장** — tau + curve + C1 suppressor 조건 + C2 slice_thresholds 통합 저장 | ✅ 구현 | 오프라인 최적화 → 운영 정책 연결 |
| C3 | CatBoost 비교 + OOF Target Encoding | 후순위 | B1 Label Encoding이 top 3 독점 중 — 추가 개선 여지 확인 후 |
| C4 | Sparse Linear + Dense Tree 2브랜치 앙상블 | 후순위 | Shape TF-IDF가 이미 중간층 기여 중 |

**Tier 3 추가 변경:** `exception_requested` 제거 (Sumologic에 없어 추론 불가)
...
| **Tier 2: 구조 개선** | 0.65~0.72 | **0.8063** | ✅ **예상 대폭 초과** |
| **Tier 3: C1/C2 + exception_requested 제거** | 0.78~0.82 | - | 서버 실험 대기 |
| Tier 4: Phase 2 (full_context_raw) | 0.85~0.90 | - | 데이터 확보 후 |

> **Tier 2 예상 vs 실측:** 예상 0.66~0.72에서 실측 0.8063. B1 범주형 피처(service/ops_dept/retention_period)의 기여가 예상보다 훨씬 컸음.
> Tier 3에서 exception_requested 제거로 소폭 하락 예상되나, C1 Suppressor가 이를 보완할 가능성.

---

## 8. Phase 1 현실적 성능 상한

Phase 1에서 사용 가능한 정보 (Sumologic 사용 가능 확인됨):
- 파일 경로/이름 → 파일 "유형" 추정
- 검출 건수/PII 유형 비율 → 대량 검출 = FP 경향
- 파일 확장자 → .log, .conf 등 시스템 파일 = FP
- 서비스/부서/조직/보관기간 (service, ops_dept, organization, retention_period)
- 서버 환경 토큰 (server_env, server_stack — server_name에서 파생)
- ~~예외 신청 이력~~ (exception_requested — Sumologic에 없음, 제거됨)
...
| 단계 | F1 목표 | F1 실측 | TP Recall 목표 | TP Recall 실측 | FP Precision 목표 | FP Precision 실측 |
|------|---------|--------|---------------|---------------|------------------|-----------------|
| Phase 1 Tier 2 | >=0.70 | **0.8063** ✅ | >=0.75 | **0.74** (근접) | >=0.85 | **0.75** (미달) |
| Phase 1 Tier 3 (C1+C2) | >=0.78 | 대기 | >=0.75 | 대기 | >=0.80 | 대기 |
...
```
=== Tier 2 Round 1+2 — ✅ 서버 실험 완료 (F1=0.8063) ===

[1] B2: 중복 샘플 가중치                              ✅
[2] B7: server_name 의미 토큰 분해                    ✅
[3] B1: 범주형 피처 Label Encoding (6개 컬럼)         ✅  (exception_requested 제외)
[4] B8: RULE 세부 신호                                ✅
[5] B9: file-level aggregation 연결                   ✅
[6] B3: Shape TF-IDF                                  ✅
[7] B6: 정규화 강화                                    ✅
[+] C5: threshold_policy.json 저장                    ✅

=== Tier 3 C1+C2 — ✅ 구현 완료 (서버 실험 대기) ===

[8] exception_requested 제거 (Sumologic에 없음)       ✅
[9] C1: Easy FP Suppressor (purity 95%+ 조건부 활성화) ✅
[10] C2: Slice-aware threshold (server_env별 tau)      ✅
```
...
### 완료 — Tier 0~1 (F1 0.6146 → 0.6114)
1. ~~`ml_tp_proba` 인덱스 역전~~ ✅
2. ~~Temporal split 월 중복~~ ✅
3. ~~시간 피처 4개 + server_freq 제거~~ ✅
4. ~~rule_matched 피처 추가~~ ✅
5. ~~TF-IDF 500→200 축소~~ ✅

### 완료 — Tier 2 Round 1+2 (F1 0.6114 → **0.8063**, 서버 실측)
6. ✅ B2: 중복 샘플 가중치 — `1/sqrt(group_size)`, mean=1 정규화
7. ✅ B7: server_env/is_prod/stack — server_name 의미 토큰 추출
8. ✅ B1: 범주형 Label Encoding — **service(1위)/ops_dept(2위)/retention_period(3위)** 가 importance top 3 독점
9. ✅ B8: RULE 세부 신호 — rule_confidence_lb, rule_id_enc, rule_primary_class_enc
10. ✅ B9: file aggregation — file_event_count, file_pii_diversity (train fold only)
11. ✅ B3: Shape TF-IDF — file_name shape 100 features (tfidf_fshape_* 중간층 기여)
12. ✅ B6: 정규화 강화 — min_child_samples=200, reg_alpha=0.5, max_depth=10
13. ✅ C5: threshold_policy.json — tau + curve + slice_thresholds 자동 저장

### 완료 — Tier 3 구현 (서버 실험 대기)
14. ✅ exception_requested 제거 (Sumologic에 없어 추론 불가)
15. ✅ C1: Easy FP Suppressor — 4개 조건 (is_system_device, is_package_path+mass, is_docker_overlay, has_license_path), purity 95%+ 조건부 활성화
16. ✅ C2: Slice-aware threshold — server_env별 tau 개별 계산
17. ✅ 진단 모듈(diagnose_data_bias.py) — 체크포인트 모델 사용으로 수정

### 남은 항목
18. C3: CatBoost + OOF Target Encoding — 후순위 (B1 Label Encoding이 이미 top 3)
19. C4: 2브랜치 앙상블 — 후순위
20. D1~D3: Phase 2 (full_context_raw) — 데이터 확보 필요
