...
| **통합 리포트** | **`scripts/run_report.py`** — 학습 체크포인트(step5/step6)를 직접 로드하여 training과 100% 동일한 피처/모델로 평가. 피처 재생성/split 재실행/독립 학습 없음. `threshold_policy.json` 우선 로드. 9-sheet Excel 출력. `--test-months` 옵션으로 temporal split 월 수 지정 (run_training.py와 동일하게 설정). 레거시 `run_poc_report.py`/`run_evaluation.py`를 대체 |
| **표준 아티팩트 저장 (Step 7+9)** | `run_training.py` `_run_label_mode()` 학습 완료 후 `models/final/`(FINAL_MODEL_DIR)에 7개 표준 아티팩트 자동 저장: `best_model_v1.joblib`, `label_encoder.joblib`, `feature_builder.joblib` (FeatureBuilderSnapshot), `ood_detector.joblib`, `calibrator.joblib` (조건부), `feature_schema.json`, `threshold_policy.json` (C5). `run_inference.py`가 기대하는 경로/형식과 일치. `§18.3` 상세 |
...
| **Tier 2 Wave 4 피처 개정** | B2 중복 가중치(`1/sqrt(group_size)`) + B7 `server_env`/`server_is_prod`/`server_stack` + B1 범주형 6개 Label Encoding (service/ops_dept/organization/retention_period/server_env/server_stack) + B8 `rule_confidence_lb`/`rule_id_enc`/`rule_primary_class_enc` + B9 `file_event_count`/`file_pii_diversity` (train fold only) + B3 Shape TF-IDF 100 + B6 정규화 강화 (min_child_samples=200, reg_alpha=0.5, max_depth=10). **10M행 실측 F1=0.8063** |
| **Tier 3 Wave 5** | `exception_requested` 제거 (Sumologic에 없어 추론 불가) + C1 Easy FP Suppressor (is_system_device/is_docker_overlay/has_license_path/is_package_path+mass 조건, purity≥95% 시 활성화) + C2 Slice-aware threshold (server_env별 tau 개별 계산) |
...
| **Phase 1 성능 실측** | Tier 2 실측 F1=**0.8063** (Baseline 0.6146→Tier0+1 0.6114→Tier2 0.8063). Feature importance top 3: service_enc(2458)/ops_dept_enc(2431)/retention_period_enc(2050). FP P/R=0.75/0.88, TP P/R=0.87/0.74. 상세: `model_performance_report.md` v7 |
