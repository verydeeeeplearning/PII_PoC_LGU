...
| **통합 리포트** | **`scripts/run_report.py`** — 학습 체크포인트(step5/step6)를 직접 로드하여 training과 100% 동일한 피처/모델로 평가. 피처 재생성/split 재실행/독립 학습 없음. `threshold_policy.json` 우선 로드. **10-sheet Excel 출력** (기존 9 + Decision Combiner 평가). Step 2b에서 ML 예측 + RuleLabeler 결과를 `combine_decisions()`로 결합하여 RULE+ML 통합 성능 (F1, 혼동행렬, decision_source 분포) 평가. `--test-months` 옵션으로 temporal split 월 수 지정. 레거시 `run_poc_report.py`/`run_evaluation.py`를 대체 |
| **표준 아티팩트 저장 (Step 7+8)** | `run_training.py` `_run_label_mode()` 학습 완료 후 `models/final/`(FINAL_MODEL_DIR)에 6개 표준 아티팩트 자동 저장: `best_model_v1.joblib`, `label_encoder.joblib`, `feature_builder.joblib` (FeatureBuilderSnapshot), `ood_detector.joblib`, `feature_schema.json`, `threshold_policy.json` (C5). Calibration 제거됨 |
| **FeatureBuilderSnapshot** | `src/models/feature_builder_snapshot.py` — fitted TF-IDF 벡터라이저 + dense 컬럼 + **categorical LabelEncoders** 통합 저장. `.transform(df)` 내부에서 `prepare_phase1_features()` 자동 호출 (meta/path/rule 피처 생성) + categorical encoding 자동 수행 → 추론 시 피처 동형성 보장 |
| **Tier 2 피처 구성** | B2 중복 가중치(`1/sqrt(group_size)`) + B7 `server_env`(5종)/`server_is_prod`/`server_stack`(5종) + B1 범주형 8개 Label Encoding (train+test 합본 fit) + B8 `rule_confidence_lb`/`rule_id_enc`/`rule_primary_class_enc` + B9 `file_event_count`/`file_pii_diversity` (df 추가, X_train 미주입) + B3 Shape TF-IDF 100 + B6 정규화 강화 (min_child_samples=200, reg_alpha=0.5, max_depth=10) |
| **Tier 3 C1+C2** | `exception_requested` 제거 (Sumologic에 없어 추론 불가) + C1 Easy FP Suppressor (is_system_device/is_docker_overlay/has_license_path/is_package_path+mass 조건, purity≥95% 시 활성화) + C2 Slice-aware threshold (server_env별 tau 개별 계산). Calibration(cv=3) 제거, Bootstrap CI(n=500) 제거 |
| **학습/추론 동형성** | `feature_preparer.py` 공통 피처 함수 (training/inference 통일). `FeatureBuilderSnapshot`에 categorical LabelEncoders 저장 → 추론 시 핵심 피처 NaN 해결. `run_inference.py` MLFeatureBuilder→FeatureBuilderSnapshot 전환 |
| **threshold_policy.json** | `models/final/threshold_policy.json` — Step 6c Coverage-Precision Curve 결과 + C1 suppressor 조건 + C2 slice_thresholds 통합 저장 (C5 구현) |
| **run_report.py 체크포인트 방식** | Step 1~6: 학습 체크포인트(step5/step6) 직접 로드 — 피처 재생성/split 재실행/독립 학습 없음. training과 100% 동일한 피처/모델로 평가. threshold_policy.json 우선 로드 |
| **diagnose_data_bias.py 체크포인트 방식** | `diagnose_split_robustness()`: 체크포인트 모델 사용, 독립 LightGBM 학습 제거. `diagnose_ablation()`: `--include-diagnosis` 시에만 실행 (10M행 기준 무거움) |
| **경로 상수 정리** | `src/utils/constants.py`: `FINAL_MODEL_DIR`, `CHECKPOINT_DIR`, `FIGURES_DIR`, `DIAGNOSIS_DIR`, `PREDICTIONS_DIR`, `EXPORTS_DIR` 추가. 하드코딩 경로를 상수로 통일 |
| **Phase 1 성능 실측** | Tier 2 F1=0.8063 (exception_requested 포함) → Tier 3 C1+C2 F1=**0.78** (exception_requested 제거 + Easy FP Suppressor). Baseline 0.6146→Tier0+1 0.6114→Tier2 0.8063→**현재 0.78**. 상세: `model_performance_report.md` |
