...
| **rule_stats 자동 집계** | `run_training.py` Step 4a — 매 학습 시 rule_id별 N/M 자동 집계 + Beta lb 계산 → `rule_stats.json` 갱신. 이전 값 대비 큰 편차(>0.2) 경고 |
| **PoC 판정 기준 변경** | **F1-macro ≥ 0.70 단독 기준**으로 통일. TP Recall(≥0.75), FP Precision(≥0.80)은 참고 지표로 변경. FP Precision 임계값 0.85→0.80 완화 |
| **DC 평가 로직 통일** | `run_report.py` 벡터화 DC 로직을 `decision_combiner.py` 임계값(0.85, 4-Case)과 동일하게 수정. 이전 하드코딩 0.5 제거 |
| **threshold_policy 추론 연동** | `run_inference.py`에서 `threshold_policy.json` 로드 → `combine_decisions(thresholds=...)` 전달 |
| **Train/Test 성능 비교** | `run_training.py` Step 6 후 Train/Test classification_report 병행 출력 + Gap 계산 (과적합 진단) |
| **DC PASS/FAIL** | `run_training.py` Step 6d — DC Grid Sweep 35개 조합 자동 평가 + 최적 임계값 탐색 |
